...
# for damage accumulation) for negative perturbations (deletions) 
...
################################################################################
outputFile = "enzymeDeletions.txt"
output = Array{Any, 2}(undef, 18, 4 * nDeletions + 1)
...
for i = 1 : nDeletions
    ...
    @printf("\t%s %i/%i\n", join(enzymeNames[idx, 2], ","), i, nDeletions)

    deletionSpan = ["complete", "phase 1", "phase 2", "phase 3"]

    for j = 1 : length(deletionSpan)
        ...
        status, rls, genTime, phases, usage = simulateLife!(cell, mass, sizeProportion, 
                                                    retention, timestep, 
                                                    maxTimesteps, regulationFactor, 
                                                    growthFlexibility, 
                                                    cell.fba.model[:enzymes][idx], 
                                                    "deletion", deletionSpan[j])
        ...
        output[:, 1 + (i-1)*4 + j] = [enzyme; enzymeStandard; pathways[i];  
                                      deletionSpan[j]; rls; rlsChange; enzymeChange;
