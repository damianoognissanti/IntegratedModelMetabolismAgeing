...
# read data
path <- "enzymeOverexpressions.txt"
data <- read_delim(path, "\t", col_names = TRUE, comment = "#")
colNames <- names(data)

# plot (divisions in phases)
meltedData <- melt(data[c(4,5,10,13,16)], id = c("overexpression span"))
meltedData <- subset(meltedData, !is.na(`overexpression span`))
fig <- ggplot(meltedData, aes(x = variable, y = value,  
                              fill = `overexpression span`,
                              alpha = variable)) +
  my_theme +
  geom_hline(yintercept = c(1, 6, 16, 23), color = "grey", size = 1) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(colour = `overexpression span`), size = 0.8) +
  facet_grid(`overexpression span` ~.) +
  xlab("") +
  ylab("number of divisions") +
  ggtitle("overexpression span") +
...
# plot (enzyme usages)
subset1 <- subset(data, `rls change` > 1)
fig <- ggplot(data, aes(x = `rls change`, y = `enzyme change`, 
                        color = `overexpression span`)) +
  my_theme +
...
subset2 <- subset(data, rls - 23 != 0) 
fig <- ggplot(subset2, aes(x = `enzyme(s)`,
                           y = `overexpression span`,
                           fill = `rls` - 23)) +
  my_theme +
  geom_tile() +
  ylab("overexpression span") +
  xlab("") +
